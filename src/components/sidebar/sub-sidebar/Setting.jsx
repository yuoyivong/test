            ...
            <Link href="/user-profile">
                <div className="p-2 bg-primaryCherUi rounded-2xl mt-3">
                    <div className="bg-white p-2 rounded-xl flex items-center gap-2">
                        ...
                        <div>
                            <h3 className="text-xs font-semibold text-blackUi">{session?.user?.name ? session?.user?.name : 'Rabinarayan Patra'}</h3>
                            <p className="text-xs text-lessBlackUi">{session?.user?.email ? session?.user?.email : 'rabinarayn@mega.com'}</p>
                        </div>
                    </div>
                </div>
            </Link>
