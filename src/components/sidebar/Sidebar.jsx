...
export const Sidebar = ({ triggerFunc }) => {
  ...
  const [workTab, setWorkTab] = useState(
    pathname.startsWith("/question") ? "qna" : "note"
  );
  ...
  useEffect(() => {
    const handleWorkspaceDeleted = () => {
      setWorkspaces([...documentsData.payload]);
    };

    eventBus.on("workspaceDeleted", handleWorkspaceDeleted);

    return () => {
      eventBus.events["workspaceDeleted"] = eventBus.events[
        "workspaceDeleted"
      ].filter((callback) => callback !== handleWorkspaceDeleted);
    };
  }, []);

  const firstWorkspaceId =
    workspaces.length > 0 && workspaces[0] ? workspaces[0].id : null;
    ...
    <div className="bg-white h-screen w-full flex flex-col justify-between p-6 xl:pb-6 pb-[env(safe-area-inset-bottom)] !z-[2000]">
      <div className="space-y-6 w-full h-full overflow-y-auto">
            ...
            <>
              <Link
                href={
                  workspaces.length === 0
                    ? "/notes/workspace"
                    : `/notes/workspace/${firstWorkspaceId}`
                }
              >
                <Image
                  src={logo}
        ...
        <SwitchGG
          ...
          handleSetWorkTab={handleSetWorkTab}
        />
        {workTab !== "qna" ? (
          <TabNote triggerFunc={triggerFunc} />
        ) : (
          <TabQna triggerFunc={triggerFunc} />
        )}
      </div>
