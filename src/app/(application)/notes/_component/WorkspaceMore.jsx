...
const WorkspaceMore = ({ workspace, deleteWorkspace }) => {
  ...
  return (
    <div ref={dropdownRef} className={"mt-5"}>
      ...
      {isOpen && (
        <div
          className="absolute z-50 right-[7.5rem] mt-2 border w-36 xl:min-w-48 bg-white shadow-md rounded-radiusBorder"
          onClick={(e) => e.stopPropagation()}
        >
          {" "}
          {/* Stop dropdown toggle when clicking inside */}
          <ul className="py-2 text-xs sm:text-sm xl:text-base">
            <li onClick={() => handleAction("invite")}>
              <InviteWorkspace />
            </li>
            <li onClick={() => handleAction("rename")}>
              <RenameWorkspace />
            </li>
