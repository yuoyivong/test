...
const ShareDocument = ({ onClose, docStatus, docId, updateDocumentStatus }) => {
    ...
    const link = 'http://localhost:3000/public-document/1fddddddddddddddddddddddddddddddddddddddddd';
                ...
                <DialogHeader>
                    ...
                    <DialogDescription>
                        <div className="relative w-full">
                            ...
                            <div className={'w-full rounded-lg xl:rounded-radiusBorder h-12 bg-primaryCherUi'}>
                                <Link1
                                    onClick={copyLink}
                                    size="20"
                                    color={status === 'private' ? "#98A2B3" : "#000000"}
                                    className={`absolute right-2 top-1/2 transform -translate-y-1/2 ${status === 'public' ? 'cursor-pointer' : 'cursor-not-allowed'}`}
                                />
                                ...
                                status === "public"
                                    ? (
                                        <button
                                            ...
                                            className={`w-32 py-3 border-lessBlackUi border-2 bg-none rounded-xl text-blackUi cursor-pointer text-xs sm:text-sm xl:text-base font-medium `}>Unpublish
                                        </button>
                                    )
                                    : (
                                        <button
                                            onClick={() => handleStatusChange('public')}
                                            className={`w-32 py-3 border-blackUi border-2 bg-blackUi rounded-xl text-white cursor-pointer text-xs sm:text-sm xl:text-base font-medium`}>Publish
                                        </button>
                                    )
